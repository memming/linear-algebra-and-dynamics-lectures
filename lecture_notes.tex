...
\textbf{Strictly upper triangular matrices} have the following zero entry pattern ($X$ marks arbitrary value):
...
\begin{tcolorbox}
    \reca Transpose of a matrix (or vector) is defined as $(A\trp)_{i,j} = A_{j,i}$.
\end{tcolorbox}
\begin{tcolorbox}
    \funfact Two vectors $\vu$ and $\vv$ are orthogonal if $\vu\trp \vv = 0$.
\end{tcolorbox}
\begin{tcolorbox}
\funfact $\vv \in \reals^{n \times 1}$ is a unit vector if $\norm{\vv} = \sqrt{\vv\trp \vv} = 1$, i.e., it is of unit length.
\end{tcolorbox}
...
\question Let the column vectors of the square matrix 
$V = \begin{bmatrix} \vv_1 & \vv_2 & \cdots & \vv_n \end{bmatrix}$
form an \textit{orthonormal basis} of $\reals^n$, that is, 
(1) any vector $\vx \in \reals^{n \times 1}$ can be expressed as $\vx = \sum_{i=1}^n \alpha_i \vv_i$ where $\alpha_i$ are scalar,
(2) if $i \neq j$, then $\vv_i$ and $\vv_j$ are orthogonal,
and
(3) $\vv_i$ is a unit vector.
