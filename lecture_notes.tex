...
If we define
\begin{align}
    \vx(t) &= \begin{bmatrix} x(t) & x^{(1)}(t) \end{bmatrix}
\end{align}
then we can transform the second order equation into
\begin{align}
    \dot{\vx}(t) &= F \vx(t)
\end{align}
by overloading the differential operator so that $\dot{\vx}(t) = \begin{bmatrix} \dot{x}(t) & \ddot{x}(t) \end{bmatrix}^\top$, and
\begin{align}
    F = \begin{pmatrix} 0 & 1 \\ -a_1 & -a_2 \end{pmatrix}
\end{align}
