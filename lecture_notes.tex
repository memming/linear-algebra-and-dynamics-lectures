...
The spectral theorem is nice because it decouples each dimension, but can we extend such analysis to more matrices?
For all normal matrices, we can diagonalize them using their eigenvectors, as long as there are $n$ linearly independent eigenvectors.
\begin{align}\label{eq:eigendecomposition}
A = S \Lambda S\inv
\end{align}

Dynamics decomposed onto the eigenmodes behave independently!
...
\question Explain how two eigenmodes that independently exhibit exponential (geometric) decay can manifest in seemingly transient amplification in a different reference frame.
\vspace{\stretch{0.5}}
...
\subsection{Non-normal matrix}
Recall that non-normal matrices do not have a complete set of orthonormal eigenvectors.
Take two (column) vectors, $\vv_1$ and $\vv_2$ that are independent (not a scalar multiple of each other).
Let $S = [\vv_1, \vv_2]$ and $\Lambda = \diag(\lambda_1, \lambda_2)$.
\question Show that $\vv_1$ and $\vv_2$ are the eigenvectors of $A = S \Lambda S\inv$ with corresponding eigenvalues $\lambda_1$ and $\lambda_2$.
\vspace{\stretch{0.5}}

As long as the set of eigenvectors span the whole space, the above construction works.
...
\end{align}
this also implies that $\fundamentalSolution(0, 0) = I$.  In many cases $\fundamentalSolution(t, 0)$ is unwieldly to actually compute.  However, we are fortunate in that we are working with differential equations of constant coefficients; in this particular case, 
\begin{align}
...
\end{align}
% \begin{align}
%     \fundamentalSolution(t, 0) = \begin{pmatrix} \phi_1(t, 0) & 0 & 0 & \cdots & 0 \\
%     0 & \phi_2(t, 0) & 0 & \cdots & 0\\
%     \vdots & \vdots & \vdots & \ddots & \vdots\\
%     0 & 0 & 0 & \cdots & \phi_N(t, 0) \end{pmatrix}
% \end{align}
