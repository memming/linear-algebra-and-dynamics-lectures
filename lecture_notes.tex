...
Much of the nonlinear dynamical systems can be understood piece by piece using our knowledge of linear dynamical systems.
In particular, the dynamics around equilibria (fixed points) are topologically conjugate to that of the linearization.
Let $\bar{x}$ be a fixed point of a (time-invariant and autonomous) nonlinear dynamical system $\dot{x} = f(x)$, that is, $f(\bar{x}) = 0$.
Let $[Df]_{i,j}(\bar{x}) = \frac{\partial f_i}{\partial x_j}\bigr\rvert_{\bar{x}}$ be the Jacobian, i.e., linearization of the dynamics at $\bar{x}$.
