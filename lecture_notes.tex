...
Quick terminology:
A matrix is \textbf{symmetric} if $A = A\trp$. A complex matrix is \textbf{Hermitian} (or \textbf{self-adjoint}) if $A = A\ctrp$.

\begin{tcolorbox}[colback=black!1!,title=Spectral theorem of linear algebra]
Any \textit{real symmetric matrix} $A$ can be decomposed into the following form,
...
Similarly, every Hermitian matrix can be decomposed into $A = U \Lambda U\ctrp$ where $U$ is a unitary matrix and $\Lambda$ a complex-valued diagonal matrix.
\end{tcolorbox}
...
\part What is $W^n$?
\vspace{\stretch{0.2}}
\part What is the general form of $\vu_i\trp \vx(t)$ in the absence of input ($I(t) = 0$)?
\vspace{\stretch{0.5}}
\part What is the general form of $\vx(t)$ with input?
\vspace{\stretch{0.5}}
\end{parts}

Note that $U$ and $\Lambda$ are related to eigenvalues and eigenvectors.
...
\begin{align}\label{eq:eigenvalue}
	A \vx = \lambda_i \vx,
\end{align}
are called the eigenvectors ($\vx_i$) and their eigenvalues ($\lambda_i$) of $A$.
\end{tcolorbox}

\question Given a spectral decomposition of $A$, show that $\vu_i$ is an eigenvector with eigenvalue $\Lambda_{i,i}$ of $A$.
\vspace{\stretch{0.3}}
...
\begin{tcolorbox}[colback=black!1!,title=Normal matrix]
If a matrix $A$ commutes with $A\trp$ (or $A\ctrp$), it is called \textbf{normal}:
\begin{align}\label{eq:normal}
	A A\ctrp = A\ctrp A
\end{align}
Normal matrices are exactly those that have \emph{a complete set of orthonormal eigenvectors}.
\end{tcolorbox}
Normal matrices include symmetric, skew-symmetric, orthogonal, and more.

\question Show that if $A$ is normal, so is $A + \sigma I$ for any $\sigma \in \reals$.
\vspace{\stretch{0.3}}
