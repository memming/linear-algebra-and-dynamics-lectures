...
Physical systems are unarguably well described by a continuous time.
...
\begin{align}
    ...
    \implies &x(t) =  \exp(-at)
\end{align}
If we were not given the initial condition, $x(0)=k$, this is as far as we could go in determining a solution.  Fortunately, we can solve for $c_1$ by using the initial condition, so that $c_1 = k$.  Thus, the full solution to the differential equation is just $x(t) = x_0 \exp(-at)$.  
