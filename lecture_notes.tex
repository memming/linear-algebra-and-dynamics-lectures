...
\runningheadrule
\runningheader{INDP 2023}{Linear Dynamics}{Park \& Dowling}
...
Show that $B = M A M\inv$ is the new dynamics matrix for $\vy_t$.
...
\begin{tcolorbox}
	...
	&=
	    ...
	    \right)
	    \right)
	    %\quad \text{(series expansion)}
	    \nonumber
%	=
%	    e^{-t/\tau_1}
%	    \left
%	    (
%	    t \left(\frac{1}{\tau_2}-\frac{1}{\tau_1}
%		\right)
%	    + O(t^2)
%	    \right
%	    )
...
A system is \textbf{time-invariant} if its response to a certain signal does not depend on (absolute) time.
...
A system is \textbf{linear}, if linear combination of input leads to corresponding linear combination of outputs,
\begin{align}
...
\end{align}
\textbf{Remark}: A matrix in this form is called a \textit{companion form matrix}.
They arise when we transform an $N$-th order differential equation into a first order system as we did above.
...
% The similarity transform is given in Ding 2010 as $c\trp[A, A^2, \ldots, A^{n-1}]$ for some vector $c$.
...
\begin{tcolorbox}[colback=black!1!,title=Properties of the matrix exponential]
    ...
    \textbf{You commute with your matrix exponential}: $A \exp(A) = \exp(A) A$
    \newline
...
Note that given a trajectory (solution) of the initial value problem of a time-varying linear dynamical system,
the time-derivative of the trajectory is also a solution.
This means that the trajectories are smooth.
Also, this means that the sensitivity follows the same dynamics as the linear dynamical system and has implications in gradient based learning of nonlinear dynamical systems~\cite{Park2023a}.
\subsection{Jordan form}
Since the matrix exponential is defined in terms of an infinite series, we are left wondering whether it can be computed analytically.
For this, we have to introduce the concept of a \textit{Jordan matrix}.
A matrix $J \in \reals^{M\times M}$ is said to be a Jordan matrix if it takes the form:
\begin{align}
    ...
    \end{pmatrix}
\end{align}
which means we could write that $J = \lambda I + N$, where matrix $N$ is nilpotent.

\begin{tcolorbox}[colback=black!1!,title=Nilpotent matrices]
    A matrix $N$ is \textbf{nilpotent} if there exists an integer $k$ such that $N^k = 0$
\end{tcolorbox}
...
In other words, the topological stability structures expressible by LDS are too limited.

\question Sketch the phase portrait of some 2D dynamical system that can stably sustain in one of two ``states'', i.e., \emph{bistable}.

However, in practice, even well trained experimental subjects only perform tasks for a limited amount of time.
The existence of an upper bound in duration introduces a potential identifiability problem in inferring the underlying dynamics as neural computation, especially if the following conjecture is true.
\begin{tcolorbox}[colback=red!10!white,title=Universal latent LDS conjecture]
...
Often we want to solve the equation
\begin{align}
    A \vx = \vy
\end{align}
for $\vx$ when we know $\vy$.  If $A$ is a square matrix of full rank, then given a $\vy$ we can always find an $\vx$ that makes $A\vx = \vy$ true.  In most applications of interest, $A$ is a tall and skinny matrix and except in select cases there will not exist an $\vx$ making $A\vx = \vy$

\begin{align}
    \framebox[1.0cm]{\clapp{\raisebox{0pt}[1.5cm][1.5cm]{$\mat A$}}\subdims{-2.0cm} n m} \ 
    \framebox[0.5cm]{\clapp{\raisebox{0mm}[4.5mm][2.5mm]{$\,\,\mat x$}}       \subdims{-8mm} m 1}
    =
    \framebox[0.5cm]{\clapp{\raisebox{0pt}[1.5cm][1.5cm]{$\mat y$}}\subdims{-2.0cm} n 1}
\end{align}
\vspace{5pt}
\begin{tcolorbox}[colback=black!1!,title=A motivating example]
    Consider the case where we want to solve $A\vx = \vy$ for the system
    \begin{align}
        \begin{pmatrix} 1 & 0 \\ 2 & 3 \\ 3 & 1 \end{pmatrix} \begin{bmatrix} \vx_1 \\ \vx_2 \end{bmatrix} = \begin{bmatrix} 2 \\ 0 \\ 0 \end{bmatrix}
    \end{align}
    The columns of the matrix $A$ span a plane in $\reals^3$, or in other words, linear combinations of the columns of $A$ can only reach vectors that lay on that plane.  If you were to randomly pick points $\vy \in \reals^3$ they would almost never lay on this plane!

    We need to adjust our thinking a bit now. Our best bet, is to find some $\hat{\vx}$ that \textit{approximately} solves $A\hat{\vx} = \vy$. How do we do this? We will define the quality of our approximate solution by the norm of the error, e.g.
    \begin{align}
        \ell = || A \hat{\vx} - \vy ||^2
    \end{align}
    The closer $A \hat{\vx}$ is to $\vy$, the better the solution $\hat{\vx}$ is.  The most immediate question is, how do we find this $\hat{\vx}$. We might also ask if $\hat{\vx}$ is unique. It turns out this $\hat{\vx}$ is unique, but what we will discuss now, is how to find this $\hat{\vx}$.
\end{tcolorbox}
Geometrically, we see that the best solution which lies in the columns of $A$ , e.g. $\hat{\vx}$, is the one that is orthogonal to the vector we would get by drawing the shortest line from $\vy$ to the plane spanned by the columns of $A$, e.g. $A\hat{\vx} - \vy$ (draw a picture!).  

From this condition, we should have that
\begin{align}
    A^\top (\vy - A \hat{\vx}) = 0
\end{align}
which if we work through the algebra, tell us
\begin{align}
    &A^\top \vy - A^\top A \hat{\vx} = 0\\
    \implies &\hat{\vx} = (A^\top A)^{-1} A^\top \vy
\end{align}
Great, now we have found $\hat{\vx}$, the best solution of the least squares problem, e.g. the vector in the plane spanned by the columns of $A$ that is orthogonal to the vector $\vy$. Lets take a closer look at this solution, e.g, the value $\hat{\vy}$ that we get by plugging in $\hat{\vx}$.
\begin{align}
    \hat{\vy} &= A \hat{\vx}\\
    &= A (A^\top A)^{-1}A^\top \vy
\end{align}
What can we say about this matrix $A(A^\top A)^{-1} A^\top$ ? Precisely, what it is doing is taking $\vy$ (which does not live in the column space of $A$) and \textit{projecting} it onto the column space of $A$.  For this reason, we call a matrix like $A(A^\top A)^{-1} A^\top$ a \textbf{projection matrix}.  

What do we expect from this projection matrix? Like we first said, it takes a vector not in the column space of $A$ and then tells us how to linearly combine columns of $A$ to get the best approximation in $A$'s column space.  You might suspect that if a vector already lives in the column space of $A$, then the projection matrix should return that vector. Lets verify this, by first noting that if $\vv$ lives in the columns space of $A$, then we can write $\vv = A \vu$ so that
\begin{align}
    A(A^\top A)^{-1} A^\top \vv &= A(A^\top A)^{-1} A^\top \left( A \vu \right)\\
    &= A(A^\top A)^{-1} A^\top A \vu\\
    &= A (A^\top A)^{-1} (A^\top A) \vu\\
    &= A \vu\\
    &= \vv
\end{align}
Another property we might suspect is that this projection matrix applied multiple times should just be itself. After all, if you project something into a particular subspace, and then project that projection you should remain in the subspace.  So, lets define $P = A(A^\top A)^{-1} A^\top$, then
\begin{align}
    P^n &= \left( A(A^\top A)^{-1} A^\top \right)^n\\
    &= A(A^\top A)^{-1} A^\top \cdot A(A^\top A)^{-1} A^\top \cdots A(A^\top A)^{-1} A^\top\\
    &= A(A^\top A)^{-1} A^\top\\
    &= P
\end{align}
\section{PCA}
Imagine we perform an experiment $N$ times where we record $M$ neurons each trial.  We then assemble the data into a matrix $X \in \reals^{N \times M}$
\begin{align}
    X = \begin{pmatrix} 
        \rule[.5ex]{3.5em}{0.4pt} & \vx^{(1)} & \rule[.5ex]{3.5em}{0.4pt} \\
        \rule[.5ex]{3.5em}{0.4pt} & \vx^{(2)} & \rule[.5ex]{3.5em}{0.4pt} \\
         & \vdots & \\
         \rule[.5ex]{3.5em}{0.4pt} & \vx^{(N)} & \rule[.5ex]{3.5em}{0.4pt}
     \end{pmatrix}
\end{align}
We do not know exactly which neurons we are recording from, or even which ones are important for the task at hand; but we do know that each experiment is costly and we want to collect as much data as we can.  After the experiment, we want to maximally compress the data with minimal loss of information.  The simplest procedure we can do is linearly project $X \in \reals^{N \times M}$ to $Y \in \reals^{N \times D}$ with $D \ll M$; this means we want to find a matrix $P$ such that
\begin{align}
    Y = X P
\end{align}
It would also be nice if the columns of $P$ were orthonormal to one another so that we would have $P^\top P = I$ (actually this ends up being necessary to make sure the optimization problem is not ill posed). We could formulate this as an optimization problem
\begin{align}
    &P = \text{arg max } \tr((XP)^\top (XP))\\
    &\text{subject to } P^\top P = I
\end{align}
Decomposing $X=USV^\top$ the $P$ that solves this optimization problem is given by $V_{1:D}$, making $Y=U_{1:D} S_{1:D}$, where the subscript $1:D$ means that we are taking the $D$ singular vectors associated with the top $D$ simgular values.  Therefore, if you want to do PCA you only have to be able to take the SVD of the data matrix $X$ and then isolate the columns of $U$ and $V$ associated with the top $D$ singular values.

\section{Observability and Controllability}
\subsection*{Observability}
Lets think about a state-space model that we write in discrete time as
\begin{align}
    \vx(t+1) &= A \vx(t) + B \vu(t)\\
    \vy(t) &= C \vx(t) + D \vu(t)
\end{align}
with $\vx(t) \in \reals^L$, $\vy(t) \in \reals^N$, and $\vu(t) \in \reals^M$. In our case $\vy(t)$ may represent neural responses that we record, $\vu(t)$ may be some experimental stimuli, and $\vx(t)$ is a time varying quantity which we are concerned with.  Usually, $\vy(t)$ will be much higher dimensional than $\vx(t)$, and our goal will be to estimate our quantity of interest, $\vx(t)$.

A good question may be under what circumstances can we perfectly recover $\vx(t)$? After all, if we use this model to try to make some scientific conclusions, this will be extremely important.  This is really the idea behind a concept called \textbf{observability}.

To try to glean some insight into this problem, the first thing we can do is write $\vy(t)$ in terms of the initial state vector $\vx(0)$.  When we do this, we find that
\begin{align}
    \vy(t) &= CA^t \vx(0) + \sum_{i=0}^{t-1} CA^{t-(i+1)} B u(i) + D u(t)
\end{align}
Lets now define a quantity that we know regardless of knowing the initial state, $\vx(0)$
\begin{align}
    \tilde{\vy}(t) &= \vy(t) - \sum_{i=0}^{t-1} CA^{t-(i+1)} B u(i) + D u(t)\\
    &= CA^k \vx(0)
\end{align}
If we actually knew $\vx(0)$, then we could recover the latent trajectory $\vx(1), \vx(2), \ldots, \vx(t)$ perfectly.  However, in this case we do not; so under what circumstances can we recover $\vx(0)$ \textit{perfectly}.  Lets now define the so-called \textit{observability matrix}, 
\begin{align}
    \mathcal{O}_t &= \begin{pmatrix} C \\ CA \\ CA^2 \\ \vdots \\ CA^{t-1} \end{pmatrix} \in \reals^{Nt \times L}
\end{align}
If we create the vector $\mathcal{Y}_{0:t-1} \in \reals^{Nt}$, defined as
\begin{align}
    \mathcal{Y}_{0:t-1} &= \begin{pmatrix} \tilde{\vy}(0) \\ \tilde{\vy}(1) \\ \vdots \\\tilde{\vy}(t-1) \end{pmatrix}
\end{align}
Then from earlier we know that we can write,
\begin{align}
    \mathcal{Y}_{0:t-1} &= \mathcal{O}_t \vx(0)
\end{align}
Now we see the exact condition for when we can recover $\vx(0)$, which is exactly when $\mathcal{O}_t$ has full column rank $L$. 

\subsection{Controllability}
Lets consider a different problem now: can we always apply a control sequence $\vu(0), \vu(1), \ldots, \vu(t)$ that can bring our initial condition from $\vx(0)$ to $0$ in a finite amount of time.  Similar to before, we can write $\vx(t)$ in terms of $\vx(0)$ as
\begin{align}
    \vx(t) &= A^t \vx(0) + \sum_{i=0}^{t-1} A^{t - (i+1)} B \vu(i)
\end{align}
then if we make the manipulation
\begin{align}
    \vx(t) - A^t \vx(0) &= \sum_{i=0}^{t-1} A^{t - (i+1)} B \vu(i)
\end{align}
we can define the so-called \textbf{controllability} matrix
\begin{align}
    \mathcal{C}_t = \begin{pmatrix} B & AB & A^2B & \cdots & A^{t-1}B \end{pmatrix}
\end{align}
and vectorize the sequence of control inputs to
\begin{align}
    \mathcal{U}_t &= \begin{pmatrix} \vu(t-1) \\ \vu(t-2) \\ \vdots \\ \vu(0) \end{pmatrix}
\end{align}
then similar to before we can write
\begin{align}
    \vx(t) - A^t \vx(0) &= \mathcal{C}_t \mathcal{U}_t
\end{align}
Therefore, if $\mathcal{C}_L$ has rank $L$, then we can reach any $\vx(t)$ starting from $\vx(0) = 0$. In fact, there is no need to require more than $L$ time steps if this is true; if we cannot do this in $L$ steps, then we can never hope to do it.
