...
\begin{align}
    \mathcal{C} = \begin{pmatrix} B \\ AB \\ A^2B \\ \vdots \\ A^{t-1}B \end{pmatrix} \in \reals^{L \times Mt} 
\end{align}
...
\begin{align}
    \vx(t) - A^t \vx(0) &= \mathcal{C} \mathcal{U}_t
\end{align}
Therefore, if $\mathcal{C}$ has column rank $L$, then we can reach any $\vx(t)$ starting from $\vx(0) = 0$. In fact, there is no need to require more than $L$ time steps if this is true; if we cannot do this in $L$ steps, then we can never hope to do it.
