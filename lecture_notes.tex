...
\clearpage
\question Show that the eigenvalues of a rotation matrix are of the form $e^{\pm i\theta}$.
\begin{flalign}
    &
    W_\theta =
    \begin{bmatrix}
...
\begin{tcolorbox}[colback=black!1!,title=Fundamental Solution]
    \begin{theorem}[Series representation of fundamental solution]
        ...
        \end{align}
        converges uniformly to $\Phi(t, 0)$. See proof in~\cite{brockett2015finite}
    \end{theorem}
...
\begin{tcolorbox}[colback=black!1!,title=Jordan normal form]
    ...
    \begin{align}
        A = T J T^{-1}
    \end{align}
    where $J$ is in Jordan normal form, and $T$ contains the generalized eigenvectors of $A$.
\end{tcolorbox}

\subsection{Algebraic and geometric multiplicity}
The \textbf{algebraic multiplicity} of an eigenvalue is the numbe
